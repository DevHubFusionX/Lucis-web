...
export default function ActionCard({ icon: Icon, label, subtext, buttonText, buttonStyle, onClick }) {
  return (
    <div className="bg-white p-6 rounded-lg border hover:shadow-lg hover:-translate-y-1 transition-all duration-300 flex flex-col h-full" style={{ borderColor: theme.colors.gray[200] }}>
      ...
      <button onClick={onClick} className="inline-flex items-center gap-2 text-sm font-medium hover:gap-3 transition-all" style={{ color: buttonStyle?.backgroundColor || theme.colors.primary[700] }}>
        {buttonText}
        <ArrowRight size={16} />
      </button>
    </div>
